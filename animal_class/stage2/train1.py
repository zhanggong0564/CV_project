...
def train(net,epoch,train_loader,optimizer,device,criterion,train_dataset):
    ...
    for batch_item in dataprocess:
        ...
        dataprocess.set_postfix_str("train_loss:{:.4f},acc{:.2%}".format(loss.item(),acc.double()/len(train_loader)))
...
def val(net,epoch,val_loader,device,val_dataset,criterion):
    ...
    for batch_item in dataprocess:
        ...
        dataprocess.set_postfix_str("val_loss:{:.4f},acc{:.2%}".format(loss.item(),acc.double()/len(val_loader)))
    avg_loss= total_loss.item() / iterm
...
def main():
    ...
    torch.save(net, os.path.join(os.getcwd(), config.SAVE_PATH, 'Fine_model_fine_.pth'))
