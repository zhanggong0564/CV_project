...
def train(net,epoch,train_loader,optimizer,device,criterion,train_dataset):
    ...
    for batch_item in dataprocess:
        ...
        dataprocess.set_postfix_str("train_loss:{:.4f},acc{:.2%}".format(loss.item(),acc.double()/config.batch_size))
...
def val(net,epoch,val_loader,device,val_dataset,criterion):
    ...
    for batch_item in dataprocess:
        ...
        dataprocess.set_postfix_str("val_loss:{:.4f},acc{:.2%}".format(loss.item(),acc.double()/81))
    avg_loss= total_loss.item() / iterm
