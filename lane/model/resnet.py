...
class BasicBlock(nn.Module):
    def __init__(self,in_ch,out_ch,stride=1,atrous=1,downsample=None):
        ...
        self.conv1 = conv3x3(in_ch,out_ch,stride,atrous)
...
class Resnet_Atrious(nn.Module):
    def __init__(self,block,layers,atrous=None,os=16):
        super(Resnet_Atrious,self).__init__()
        stride_list  = None
        if os==8:
            stride_list=[2,1,1]
        elif os==16:
    ...
    def _make_layer(self,block,in_ch,out_ch,blocks,stride=1,atrous = None):
        downsample = None
        if atrous==None:
            atrous=[1]*blocks
        ...
        layers = []
        layers.append(block(in_ch,out_ch,stride=stride,atrous=atrous[0],downsample=downsample))
        in_ch = out_ch*4
        for i in range(1,blocks):
            layers.append(block(in_ch,out_ch,stride=1,atrous=atrous[i]))
...
def resnet50_atrous(pretrained =True,os=16,**kwargs):
    model = Resnet_Atrious(Bottleneck,[3,4,6,3],atrous=[1,2,1],os= os,**kwargs)
    if pretrained:
...
def resnet101_atrous(pretrained=True, os=16, **kwargs):
    ...
    model = Resnet_Atrious(Bottleneck, [3, 4, 23, 3], atrous=[1, 2, 1], os=os, **kwargs)
    if pretrained:
...
def test():
    ...
    model = resnet50_atrous(pretrained=False)
    model.eval()
    y = model(x)
    a,b,c,d =y
